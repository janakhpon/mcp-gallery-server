...
export class QueueService {
  ...
  constructor() {
    ...
    if (redisHost === 'redis' && !isDocker && process.env.NODE_ENV !== 'production') {
      redisHost = '127.0.0.1';
    }
    this.queue = new Bull<ImageJobPayload>('image-processing', { redis: { host: redisHost, port: redisPort } });
  }

  async enqueueImageProcessing(imageId: string): Promise<void> {
    await this.queue.add({ imageId }, { attempts: 3, backoff: { type: 'exponential', delay: 1000 } });
  }
